...
if (($_POST["HaberTuru"]) === "0") {
    $sql = "update haberler set Oncelik = ? ,Baslik = ?, UstYazi = ?, AltYazi = ?, KapakFotografi = ?, Ozet = ? where HaberID = ?";
    ...
    $stmt->bind_param("isssbsi", $Oncelik, $Baslik, $UstYazi, $AltYazi, $Null, $Ozet, $HaberID);
    $stmt->send_long_data(4, $kapakFotografi);
    $stmt->execute();
    $stmt->close();
    kategoriFotografGuncelle($HaberID);

} else if (($_POST["HaberTuru"]) === "1") {
    $sql = "update haberler set Oncelik = ?, Baslik = ?, KapakFotografi = ?, Ozet= ? where HaberID = ?";
    $HaberID = intval($_GET["id"]);
    $stmt = $conn->prepare($sql);
    $Baslik = $_POST["Baslik"];
    $Ozet = $_POST["Ozet"];
    ...
    $kapakFotografi = explode(",", $kapakFotografi);
    $kapakFotografi = base64_decode($kapakFotografi[1]);
    $stmt->bind_param("isbsi", $Oncelik, $Baslik, $Null, $Ozet, $HaberID);
    ...
    $kapakFotografi = explode(",", $kapakFotografi);
    $kapakFotografi = base64_decode($kapakFotografi[1]);
    $stmt->bind_param("isbs", $Oncelik, $Baslik, $Null, $Ozet, $HaberID);
    $stmt->send_long_data(2, $kapakFotografi);
    $stmt->execute();
    $stmt->close();
    kategoriFotografGuncelle($HaberID);
...
function kategoriFotografGuncelle($HaberID)
{
    $sil = "delete from haberkategori where HaberID = $HaberID";
    global $conn;
    global $purifier;
    global $doc;
    $conn->query($sil);
    ...
    $sql = "delete from haberlerfotograf where HaberID = $HaberID";
    $conn->query($sql);
